...
Experimental Homebridge Matter plugin using the existing pypowerwall HTTP proxy.
...
**Protocol construction and offline tests pass; commissioning and Apple Home 27 rendering have not been tested on hardware.**
Read the [research and delivery plan](docs/APPLE_HOME_RESEARCH.md) before choosing meter presentation.

## Installation

Requires Homebridge 2.4+, Node 22/24/26, and a running pypowerwall proxy.
For native energy display, target iOS/iPadOS 27 and a Home hub on version 27;
macOS 27 presentation requires separate verification. This is an unpublished local package.

```sh
cd /Users/Streicher/Developer/pypowerwall/homebridge
npm ci --ignore-scripts
npm test
npm pack
```

Install the resulting `homebridge-powerwall-meters-0.2.1.tgz` into the Homebridge
runtime's plugin directory (for example `npm install -g /absolute/path/to/the.tgz`
on a conventional global installation). Container installs must retain the package
in their persistent plugin storage. Enable **Matter** on the plugin's child bridge
and pair its **Matter QR code** with Home. This plugin publishes no HAP accessories.
...
The battery accessory is named **Low Battery Warning** and keeps its existing UUID.
...
| `backupReservePresets: [20,50,100]` | Backup Reserve 20% / 50% / 100%, configurable integer presets |
